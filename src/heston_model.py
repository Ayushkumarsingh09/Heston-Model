...
def heston_integrand(phi, S, K, T, r, kappa, theta, sigma, rho, v0, option_type):
    ...
    f = np.exp(C + D * v0 + i * phi * x) / (i * phi)
    return np.real(np.exp(-i * phi * np.log(K)) * f)
...
def heston_price(S, K, T, r, kappa, theta, sigma, rho, v0, option_type="call"):
    ...
    integral_2 = quad(lambda phi: heston_integrand(phi, S, K, T, r, kappa, theta, sigma, rho, v0, option_type), 0, np.inf)[0]
...
if __name__ == "__main__":
    ...
    call_price = heston_price(S, K, T, r, kappa, theta, sigma, rho, v0, "call")
